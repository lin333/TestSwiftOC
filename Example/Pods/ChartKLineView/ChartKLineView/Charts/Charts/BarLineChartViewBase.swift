...
{
    @objc optional func customDrawLocationChanged(_ location: CGPoint)
    @objc optional func customDrawActionCompleted(_ customData: CustomDrawChartData, dataSet: CustomChartDataSet)
    @objc optional func customDrawDataSetDidSelected( _ dataSet: CustomChartDataSet)
...
open class BarLineChartViewBase: ChartViewBase, BarLineScatterCandleBubbleChartDataProvider, NSUIGestureRecognizerDelegate
{
    ...
    /// top axis.
    @objc open public(set) var leftAxis = YAxis(position: .left)
    
    /// The right y-axis object. In the horizontal bar-chart, this is the
    /// bottom axis.
    @objc open public(set) var rightAxis = YAxis(position: .right)
    ...
    @objc open var _leftAxisTransformer: Transformer!
    @objc open var _rightAxisTransformer: Transformer!
    ...
    @objc open var _tapGestureRecognizer: NSUITapGestureRecognizer!
    @objc open var _doubleTapGestureRecognizer: NSUITapGestureRecognizer!
    #if !os(tvOS)
    @objc open var _pinchGestureRecognizer: NSUIPinchGestureRecognizer!
    #endif
    @objc open var _panGestureRecognizer: NSUIPanGestureRecognizer!
    ...
    ///当前是否处于划线状态，用于判断各种手势
    private var _drawingCustomLine: Bool = false
    @objc open var drawingCustomLine: Bool {
        set {
            _drawingCustomLine = newValue
        }
        get {
            return _drawingCustomLine
        }
    }
    
    ///当前正在编辑的dataSet，用于整个移动划线内容
    private var _drawingDataSet: CustomChartDataSet?
    @objc open var drawingDataSet: CustomChartDataSet? {
        get {
            return _drawingDataSet
        }
        set {
            //画线状态
            if _drawingCustomLine {
                ///如果有编辑状态就取消编辑
                if _editingCustomLine {
                    editingCustomLine = false
                }
                ///设置新画线数据
                _drawingDataSet = newValue
                ///设置当前为高亮
                _drawingDataSet?.customDrawSelected = true
            } else {
            ///编辑状态
                if _editingCustomLine {
                    ///取消上一个编辑状态
                ...
                ///如果有更多的线，把当前编辑的放到最前面
                if customLineData.dataSetCount > 1 && _drawingDataSet !== nil  {
                    customLineData.dataSets.swapAt(0, customLineData.indexOfDataSet(_drawingDataSet!))
                }
            }
            if _drawingDataSet !== nil {
                perfromCustomDrawDataSetDidSelectedDelegate()
            } else {
                perfromCustomDrawDataSetDesSelectedDelegate()
            }
        }
    }

    ///当前是否处于编辑自定义划线
    private var _editingCustomLine: Bool = false
    @objc open var editingCustomLine: Bool {
        get {
            return _editingCustomLine
        }
        set {
            _editingCustomLine = newValue
            if !newValue {
                _drawingDataSet?.customDrawSelected = false
                _drawingEntry = nil
                _drawingDataSet = nil
            } else {
                _drawingDataSet?.customDrawSelected = true
                _drawingCustomLine = false
            }
            setNeedsDisplay()
        }
    }
    
    ///当前正在编辑的Entry，用于移动某个entry达到重绘的目的
    private var _drawingEntry: CustomChartDataEntry?
    @objc open var drawingEntry: CustomChartDataEntry? {
        get {
            return _drawingEntry
        }
        set {
            _drawingEntry = newValue
        }
    }

    ///承载绘图的数据
    @objc open lazy var customLineData = CustomDrawChartData()
    /// 绘图的撤销前进操作
    public var customChartDataUndoManager = UndoManager()
    ...
    @objc open func prepareValuePxMatrix()
    {
    ...
    @objc open func prepareOffsetMatrix()
    {
    ...
    open func calculateLegendOffsets(offsetLeft: inout CGFloat, offsetTop: inout CGFloat, offsetRight: inout CGFloat, offsetBottom: inout CGFloat)
    {
    ...
    @objc open func drawGridBackground(context: CGContext)
    {
    ...
    @objc private func tapGestureRecognized(_ recognizer: NSUITapGestureRecognizer)
    {
        ...
        if recognizer.state == NSUIGestureRecognizerState.ended
        {
            ...
            if customDrawInterruptGestureWithTouchPoint(recognizer.location(in: self)) {
                return
            }
    ...
    @objc private func panGestureRecognized(_ recognizer: NSUIPanGestureRecognizer)
    {
        if recognizer.state == NSUIGestureRecognizerState.began && recognizer.nsuiNumberOfTouches() > 0
        {
            ...
            ///如果当前处于编辑状态，进行判断
            if customDrawInterruptGestureWithTouchPoint(_movePreviousPoint) {
                return
            }
            delegate?.chartViewBeginPanning?(self)
        ...
        {
            ///如果命中了当前点亮位置，就进行移动点亮视图，拦截后面的事件
            if customDrawInterruptPanGestureWithTouchPoint(_movePreviousPoint) {
                ///如果点亮位置可以移动成功，则打断后面的操作，并且更新移动点的位置
    ...
    @objc private func decelerationLoop()
    {
        ...
        if !performPanChange(translation: distance)
        {
            ...
            if viewPortHandler.touchMatrix.tx != 0
            {
                let notificationName = Notification.Name(rawValue: "NotificationKlineScrollToEndKey")
                NotificationCenter.default.post(name: notificationName, object: "")
            }
        }
    ...
    private func nsuiGestureRecognizerShouldBegin(_ gestureRecognizer: NSUIGestureRecognizer) -> Bool
    {
        ...
        if gestureRecognizer is NSUIPanGestureRecognizer {
            let panLocationEditingDataSet = customDrawInterruptGestureWithTouchPoint(gestureRecognizer.location(in: self))
            
            if gestureRecognizer == _panGestureRecognizer
            {
                ///如果当前触摸点命中了自定义画线的数据，则需要继续执行手势，不必进行后续判断
                ...
                ///其他pan手势需要判断是否命中了点亮数据，如果命中了就禁止掉，例如pageControlSegment切换视图，但是外部也会使用代理判断，不太准确
        ...
        else
        {
            ...
            #if !os(tvOS)
            ///在点亮状态下，取消长按手势支持
            if gestureRecognizer is UILongPressGestureRecognizer && touchInCustomLine {
                return false
            }
            #endif
        }
