    ...
    open override func drawValues(context: CGContext)
    {
        ...
        for i in 0 ..< dataSets.count
        {
            ...
            for j in 0 ..< dataSet.entryCount
            {
                ...
                let valueTextRect = (valueText as NSString).boundingRect(with: CGSize(width: 200, height: 100), options: .usesLineFragmentOrigin, attributes: [.font: valueFont], context: nil)
                ...
                if drawXOutside || drawYOutside
                {
                    ...
                    //斜边长 pt1 -> center
                    let pt1Radius = sqrt(pow(pt1.x - center.x, 2) + pow(pt1.y - center.y, 2));
                    ...
                    if transformedAngle.truncatingRemainder(dividingBy: 360.0) >= 90.0 && transformedAngle.truncatingRemainder(dividingBy: 360.0) <= 270.0
                    {
                        ...
                        if (!drawXOutside && drawYOutside && preEdge != .zero) {
                            let currentBottom = labelPoint.y + lineHeight / 2
                            if currentBottom > preEdge.top {
                                pt1 = CGPoint(x: pt1.x, y: pt1.y - abs(currentBottom - preEdge.top))
                                if (pt1.y >= center.y - pt1Radius && pt1.y <= center.y + pt1Radius) {
                                    pt1.x = center.x - sqrt(pow(pt1Radius, 2) - pow(pt1.y - center.y, 2));
                                }
                                pt2 = CGPoint(x: pt1.x - polyline2Length, y: pt1.y)
                                labelPoint = CGPoint(x: pt2.x - 5, y: pt2.y - lineHeight)
                            } else {
                    ...
                    else
                    {
                        ...
                        if (!drawXOutside && drawYOutside && preEdge != .zero) {
                            let currentTop = labelPoint.y - lineHeight / 2
                            if preEdge.bottom > currentTop {
                                pt1 = CGPoint(x: pt1.x, y: pt1.y + abs(preEdge.bottom - currentTop))
                                if (pt1.y >= center.y - pt1Radius && pt1.y <= center.y + pt1Radius) {
                                    pt1.x = center.x + sqrt(pow(pt1Radius, 2) - pow(pt1.y - center.y, 2));
                                }
                                pt2 = CGPoint(x: pt1.x + polyline2Length, y: pt1.y)
                    ...
                    else if drawYOutside
                    {
                        ...
                        let bottom = labelPoint.y + lineHeight / 2
                        ...
                        if align == .left {
                            left = labelPoint.x
                            right = left + valueTextRect.width
            
                        } else {
                            right = labelPoint.x
                            left = right - valueTextRect.width
                        }
                        preEdge = UIEdgeInsets(top: top, left: left, bottom: bottom, right: right)
                    }
                }
