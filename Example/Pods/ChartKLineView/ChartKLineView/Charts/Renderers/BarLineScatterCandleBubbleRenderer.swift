...
open class BarLineScatterCandleBubbleRenderer: DataRenderer
{
    ...
    @objc open func isInBoundsX(entry e: ChartDataEntry, dataSet: IBarLineScatterCandleBubbleChartDataSet) -> Bool
    {
    ...
    @objc open func shouldDrawValues(forDataSet set: IChartDataSet) -> Bool
    {
    ...
    open class XBounds
    {
        ...
        open func set(chart: BarLineScatterCandleBubbleChartDataProvider,
                      ...
                      animator: Animator?)
        {
            var phaseX = Swift.max(0.0, Swift.min(1.0, animator?.phaseX ?? 1.0))

            if !dataSet.animateXEnable {
                phaseX = 1.0
            }
