...
open class CombinedChartRenderer: DataRenderer
{
    ...
    public var _drawOrder: [CombinedChartView.DrawOrder] = [.bar, .bubble, .line, .candle, .scatter]
    ...
    @objc open func createRenderers()
    {
        ...
        for order in drawOrder
        {
            ...
            case .custom:
                if chart.customDrawData !== nil
                {
                    _renderers.append(CustomChartRenderer(dataProvider: chart, animator: animator, viewPortHandler: viewPortHandler))
                }
                break

            case .compare:
                if chart.compareData !== nil
                {
                    _renderers.append(StockCompareChartRenderer(dataProvider: chart, animator: animator, viewPortHandler: viewPortHandler))
                }
                break
            }
        }
    ...
    open override func drawHighlighted(context: CGContext, indices: [Highlight])
    {
        for renderer in _renderers
        {
            ...
            else if renderer is CustomChartRenderer
            {
                data = (renderer as! CustomChartRenderer).dataProvider?.customDrawData
            }
            else if renderer is StockCompareChartRenderer
            {
                //不绘制高亮
                return
            }
