...
open class ChartData: NSObject
{
    ...
    @objc open func calcMinMax()
    {
        ...
        if (_yMin == Double.greatestFiniteMagnitude || _yMax == -Double.greatestFiniteMagnitude)
        {
            _yMin = 0.0
            _yMax = 0.0
        }
    ...
    {
        guard _xMin != Double.greatestFiniteMagnitude else {
                    return 0
                }
        return _xMin
    }
    ...
    {
        guard _xMax != -Double.greatestFiniteMagnitude else {
                    return 1
                }
        return _xMax
    }
    ...
    internal func getDataSetIndexByLabel(_ label: String, ignorecase: Bool) -> Int
    {
        ...
        if ignorecase
        {
            return dataSets.firstIndex {
                guard let currentLabel = $0.label else { return false }
                return label.caseInsensitiveCompare(currentLabel) == .orderedSame
            } ?? -1
        }
        else
        {
