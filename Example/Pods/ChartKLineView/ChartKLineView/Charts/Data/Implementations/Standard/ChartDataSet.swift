...
open class ChartDataSet: ChartBaseDataSet
{
    ...
    {
        ///这里我默认是有规则的x值，如果不是的话则按 firstIndex(of:) 取值
        guard let beginX = entries.first?.x, let lastX = entries.last?.x else {
            return -1
        }

        let range = (lastX - beginX) / Swift.max(Double(1), Double(entries.count - 1))
        
        if range <= 0 || e.x < beginX || e.x > lastX {
            return entries.firstIndex(of: e) ?? -1
        }
        
        return Int((e.x - beginX) / range)
    }
    ...
    open override func copy(with zone: NSZone? = nil) -> Any
    {
        ...
        copy.entries = entries.map {
            return $0.copy() as! ChartDataEntry
        }
        copy._yMax = _yMax
        copy._yMin = _yMin
        copy._xMax = _xMax
...
extension ChartDataSet/*: Equatable*/ {
    open override func isEqual(_ object: Any?) -> Bool {
        guard let object = object as? ChartDataSet else { return false }

        if object.count != self.count {
            return false
        }
        
        for index in stride(from: 0, to: self.count - 1, by: +1) {
            let entry = self[index]
            let objectEntry = object[index]
            if !entry.isEqual(objectEntry) {
                return false
            }
        }
        return true
