...
open class CombinedChartData: BarLineScatterCandleBubbleChartData
{
    ...
    private var _customDrawData: CustomDrawChartData!
    private var _compareData: LineChartData!
    ...
    @objc open var customDrawData: CustomDrawChartData!
    {
        get
        {
            return _customDrawData
        }
        set
        {
            _customDrawData = newValue
            notifyDataChanged()
        }
    }
    
    /// 股票对比
    @objc open var compareData: LineChartData!
    {
        get
        {
            return _compareData
    ...
    open override func calcMinMax()
    {
        ...
        let allData = self.allCalculateData
...
//            _dataSets.append(contentsOf: sets)
    ...
    @objc open var allData: [ChartData]
    {
        ...
        if customDrawData !== nil {
            data.append(customDrawData)
        }
        
        if compareData !== nil {
            data.append(compareData)
        }
        
        return data
    }
    
    /// 划线部分不参与最大值最小值的计算，是固定的位置，不会影响y轴缩放
    @objc open var allCalculateData: [ChartData]
    {
        var data = [ChartData]()
        if candleData != nil {
            data.append(candleData)
    ...
    open override func notifyDataChanged()
    {
        ...
        if _customDrawData !== nil
        {
            _customDrawData.notifyDataChanged()
        }
        
        if _compareData !== nil {
            _compareData.notifyDataChanged()
        }
